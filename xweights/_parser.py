...
def create_parser():
    ...
    parser.add_argument('-i', '--input_files', dest='input_files', nargs='+', help='List of input files')
    ...
    parser.add_argument('-s', '--subregion', dest='name', nargs='+', type=str, help='Set names of the subregions')
    ...
    parser.add_argument('-column', '-column_merge', dest='column_merge', nargs='?', type=str, help='Column name to differentiate shapefile while merging.')
    parser.add_argument('-tstat', '--time_statistics', dest='time_statistics', nargs='+', help='Set additional statistics over time. E.g. mean, sum,  std, min, max,')
    parser.add_argument('-lony', '--land_only', dest='land_only', nargs='+', help='Consider only land points.')
    parser.add_argument('-kwargs', '--kwargs', dest='kwargs', nargs='*', default={}, action=ParseKwargs, help='User-given dictionary with additional settings.')
    parser.add_argument('-which_regions', '--which_regions', dest='which_regions', nargs='?', const=True, help='Print available regions on screen.')
    parser.add_argument('-which_subregions', '--which_subregions', dest='which_subregions', nargs='+', help='Print all subregions of selected region on screen.')
    return parser

parser = create_parser()
args = parser.parse_args()

if args.merge: args.column=args.merge
